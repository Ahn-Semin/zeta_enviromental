...
char recByte = '\0';
bool nc = false; // no contact detection flag
...
#endif
  InitTimersSafe(); 
  bool success = SetPinFrequencySafe(PWM_PINP, FREQUENCY);
  if(success) {
    #if DEBUG
    Serial.println("PWM set");
    #endif
    pinMode(PWM_PINP, OUTPUT);
    digitalWrite(PWM_PINP, LOW);   
  }
...
void PWM_OUT() {
  if (nc) {
    ...
    Serial.println("PWM");
#endif
    pwmWrite(PWM_PINP, PWM_WIDTH * PWM_DUTY);
    //pwmWrite(PWM_PINN, PWM_WIDTH * PWM_DUTY);
