...
const uint32_t Time_toPublish = 5;  // modify it
...
void CO_ADC() {
  static float CO_ppb_arr[CO_window_size] = {0.0,};
  static int cnt = 0;
  float sum = 0.0;
  if(cnt==CO_window_size) {
    ...
    CO_ppb = sum / (float)CO_window_size; // smoothing
    cnt = 0;
  } else {
...
void Split(String sData, char cSeparator) {  
  ...
  for(int i = 0; i<4; i++) {
    ...
    else if(i==1) Rn_nSv = sTemp.toInt();
...
void Publish_data() {
  ...
  (int)Rn_nSv,        TVOCs_ugm3,         (int)temp_celcius,
  (int)hum_RHp);  // Rn is nSv unit
  sprintf(temp,"%d", (int)Rn_nSv);
  nh.loginfo(temp);
  nh.loginfo(send_msg);
