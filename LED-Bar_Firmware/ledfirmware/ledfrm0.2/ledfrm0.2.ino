...
/*  fill_solid(leds[0],LED1_NUM, firstval);                        // A 'long' RGB value
  delay(2000);
  fill_solid(leds[0],LED1_NUM, CRGB::HotPink);                        // A 'long' RGB value
  delay(2000);
  fill_solid(leds[1], LED2_NUM, rgbval);                         // 8 bit values inside the colour definition
  delay(2000);
  fill_solid(leds[2], LED3_NUM, hsvval);                         // 8 bit values inside the colour definition
  delay(2000);

 
  fill_gradient_RGB(leds[0], 0, 0x000011, LED1_NUM-1, 0x110000);   // You can mix and match long values and CRGB values. Remember, endpos goes up to NUM_LEDS-1
  delay(2000);
 fill_gradient_RGB(leds[0], LED1_NUM, CRGB(50,0,200), CRGB(80,200,240));  // up to 4 CRGB (or long) values
 delay(2000);*/

 #if 0 
  BouncingColoredBalls(3, 500, colors, 0, LED1_NUM);
 ...
 // ExampleTotal(0, LED1_NUM);
  ...
  theaterChase(10, 0, 0xaa,0,0xaa,50,0, LED1_NUM);
  //theaterChase(10, 0, 0,0xff,0,50,0, LED1_NUM);
  ...
  //TwinkleRandom(LED1_NUM, 50, false,0,LED1_NUM);
...
void mover2(byte cnt, byte thisdelay, byte thisfade, byte indexled) { 
...
void BouncingColoredBalls(int BallCount, int count, byte colors[][3],byte indexled, byte lednum) {
  float Gravity = -9.81;
  int StartHeight = 1;
  ...
  for(int j=0; j<count; j++) {
    for (int i = 0 ; i < BallCount ; i++) {
      ...
      Position[i] = round( Height[i] * (lednum - 1) / StartHeight);
    }
