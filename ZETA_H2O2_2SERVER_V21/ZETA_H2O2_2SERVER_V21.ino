...
void setup() {
  ...
  if (WiFi.status() == WL_NO_MODULE) {
    #if DEBUG
    Serial.println("No available Wifi module!");
    #endif
    Socket_CMD("No available Wifi module!");
    // don't continue
    while (true);
  }
...
void SetSpan() {
  ...
  #if DEBUG
  Serial.println("SetSpan enter");
  #endif
  if(SocketSpan) {
    #if DEBUG
    Serial.println("SetSpan(socket) enter");
    #endif
    span_valueS = Socket();
    #if DEBUG
    Serial.print("from server in set span: "); Serial.println(span_valueS);
    #endif
    SocketSpan = false;
  ...
  #if DEBUG
  Serial.print("span value: "); Serial.println(span_value);
  #endif
...
String Socket() {
  ...
  if(!client.connect(server, port)) {
    ...
    Serial.println("Socket enter");
      ...
      #if DEBUG
      if(rec_socket != '\0'){
        Serial.print("from server: "); Serial.println(recevbline);  
      }
      #endif    
  }
  client.flush();
