...
void CCS811::readAlgorithmResults()
{
  ...
  Wire.requestFrom(_I2C_ADDR, (uint8_t)4); //Get four bytes
...
unsigned int CCS811::getBaseline()
{
  ...
  Wire.requestFrom(_I2C_ADDR, (uint8_t)2); //ITBP add		Get two bytes
...
byte CCS811::readRegister(byte addr)
{
  ...
  Wire.requestFrom(_I2C_ADDR, (uint8_t)1);  //ITBP add
